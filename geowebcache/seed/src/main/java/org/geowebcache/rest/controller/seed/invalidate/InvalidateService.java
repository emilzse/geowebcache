...
public class InvalidateService extends AbsService {
    ...
    protected ResponseEntity<?> handleRequest(String layerName, String formatExtension, String body, String jobId, int poolSize) {
        ...
        if (formatExtension==null || formatExtension.equalsIgnoreCase("xml")) {
            ...
            return new ResponseEntity<Object>("Format extension unknown or not specified: "
    ...
    protected ResponseEntity<?> handleRequest(String layerName, InvalidateRequest obj, String jobId, int poolSize) {
        try {
            GWCTask[] tasks = createTasks(seeder.findTileLayer(layerName), obj, jobId, poolSize);
            seeder.dispatchTasks(tasks);

            return new ResponseEntity<Object>(Arrays.stream(tasks).mapToLong(GWCTask::getTaskId).toArray(), HttpStatus.OK);
