...
class InvalidateTask extends GWCTask {
    private static Log log = LogFactory.getLog(org.geowebcache.rest.controller.seed.invalidate.InvalidateTask.class);
    ...
    public InvalidateTask(StorageBroker storageBroker, QuotaStore quotaStore, TileLayer tl,
            InvalidateConfig[] list, DataSource dataSource, String tableName, String jobId, int poolSize) {
    ...
    protected void doActionInternal() throws GeoWebCacheException, InterruptedException {
        ...
        if (!this.terminate) {
            ...
            List<InvalidateTileRange> tileRanges = pages.stream().map(this::initFromTilePage).collect(Collectors.toList());
    ...
    private InvalidateTileRange initFromTilePage(TilePage page) {
        ...
        try {
            return new InvalidateTileRange(page, splitted[0], gridsetId, z, z, rangeBounds, MimeType.createFromFormat(splitted[2]), parameters);
    ...
    private class DeleteStoreListener implements BlobStoreListener {
        ...
        @Override
        public void tileStored(String layerName, String gridSetId,
                String blobFormat, String parametersId, long x, long y, int z,
                long blobSize, int epsgId, double[] bbox,
        ...
        @Override
        public void tileUpdated(String layerName, String gridSetId,
                String blobFormat, String parametersId, long x, long y, int z,
                long blobSize, long oldSize) {
        }
    ...
    private void loadByCopy(final List<DeletedTile> list) throws SQLException, IOException, InterruptedException {
    ...
    private void loadServiceStats(final OutputStream out, final List<DeletedTile> list) throws IOException {
        if (log.isTraceEnabled()) {
            log.trace("Begin");
        }
        try {
            ...
            for (DeletedTile tile : list) {
                ...
                try {
                    ...
                    if (log.isWarnEnabled()) {
                        log.warn(String.format("Could not convert to %s: '%s'", encoding, e.getMessage()));
                    }
                }
            }
        } finally {
    ...
    private class InvalidateTileRange extends TileRange {
        ...
        public InvalidateTileRange(TilePage tilePage, String layerName, String gridSetId, int zoomStart, int zoomStop,
