...
public final class ResponseUtils {
     ...
     * tiles
     */
    public static void writeEmpty(DefaultStorageFinder defaultStorageFinder, ConveyorTile tile,
            String message, RuntimeStats runtimeStats) {
        tile.servletResp.setHeader("geowebcache-message", message);
        TileLayer layer = tile.getLayer();
        if (layer != null) {
            // If not set can not get tile index
            if (Objects.nonNull(tile.getStorageObject())) {
