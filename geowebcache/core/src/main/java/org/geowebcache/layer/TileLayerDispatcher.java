...
public class TileLayerDispatcher
    ...
    public synchronized TileLayerConfiguration removeGridset(final String gridSetName)
        ...
        ConfigurationDispatcher persistingConfig = null;
        for (BaseConfiguration c : configs) {
            if (c instanceof TileLayerConfiguration) {
                persistingConfig = (ConfigurationDispatcher) c;
            }
        }
        if (persistingConfig == null) {
            throw new IllegalStateException(
                    "Found no configuration of type " + ConfigurationDispatcher.class.getName());
        }
